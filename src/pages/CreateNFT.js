...
const Mint = () => {
  ...
  const addDataToBlockchain = async ({ event, category, attributes }) => {
    setStart(true);
    const fileInput = document.querySelector('input[id="certificateImg"]');
    let results;
    let outputCsvData = [];
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const outputData = {
          ...
          link: `https://award-recognition.netlify.app/TCU2022/21-10-2022/${currentTokenId}`,
        };
  ...
  const saveData = async (val, event, category, attributes, results) => {
    let responseData;
